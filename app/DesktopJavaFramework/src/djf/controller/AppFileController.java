...
import static djf.settings.AppPropertyType.EXPORT_IMAGE_EXT;
import static djf.settings.AppPropertyType.LOAD_COMPLETED_MESSAGE;
import static djf.settings.AppPropertyType.LOAD_COMPLETED_TITLE;
...
import static djf.settings.AppPropertyType.SAVE_AS_FILE_EXT;
import static djf.settings.AppPropertyType.SAVE_AS_FILE_EXT_DESC;
...
import static djf.settings.AppPropertyType.SAVE_FILE_EXT;
...
public class AppFileController {
	...
	PropertiesManager props = PropertiesManager.getPropertiesManager();
        try {
            ...
            if (continueToMakeNew) {
                ...
                if (!welcomeDialog.isRecent()) {
                    // POP UP THE DIALOG
                    textDialog.show("Create New Map", "Enter Name of the New Map to Create:");
                    
                    // THEN SAVE THE NAME OF THE MAP
                    currentWorkFileName = textDialog.getText();
                }
		...
		app.getWorkspaceComponent().activateWorkspace(app.getGUI());
		...
		currentWorkFile = null;
                ...
                // TELL THE USER THE WORK IS UNDERWAY
                if (welcomeDialog.isRecent()) {
                    dialog.show(props.getProperty(LOAD_COMPLETED_TITLE), props.getProperty(LOAD_COMPLETED_MESSAGE));
                }
                else {
	...
	PropertiesManager props = PropertiesManager.getPropertiesManager();
	    ...
	    // OTHERWISE WE NEED TO PROMPT THE USER
                ...
                // CREATE NEW FILES TO EXPORT
                File file = new File(PATH_WORK + currentWorkFileName + props.getProperty(SAVE_FILE_EXT));
    ...
    /**
     * This method will save the current course to a file but in different name. 
     * This will prompt the user for the new file name.
     * 
     */
	...
	PropertiesManager props = PropertiesManager.getPropertiesManager();
        try {
            ...
            new ExtensionFilter(props.getProperty(SAVE_AS_FILE_EXT_DESC), props.getProperty(SAVE_AS_FILE_EXT)));
    ...
    /**
     * This method will export the current work to a JSON and image file format.
     * 
     */
	...
	PropertiesManager props = PropertiesManager.getPropertiesManager();
        try {    
            // CREATE NEW FILES TO EXPORT
            File json = new File(PATH_WORK + currentWorkFileName + props.getProperty(EXPORT_FILE_EXT));
            File image = new File(PATH_WORK + currentWorkFileName + props.getProperty(EXPORT_IMAGE_EXT));

            // THEN EXPORT JSON AND IMAGE FILE
            app.getFileComponent().exportData(app.getDataComponent(), json.getPath());
            app.getFileComponent().exportImage(app.getDataComponent(), image.getPath());
    ...
    public void setFileName(String newWorkFileName) {
        currentWorkFileName = newWorkFileName;
    }
}
